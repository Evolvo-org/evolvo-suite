...
export const ProjectRuntimeMonitorPanel = ({
  ...
  if (projectQuery.isLoading) {
    return (
      <Card className="p-6" title="Loading runtime monitor">
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          Fetching runtime health and recent activity from the API.
        </p>
      </Card>
    );
  }
  ...
  if (projectQuery.isError || !projectQuery.data) {
    return (
      <Card className="p-6" title="Runtime monitor unavailable">
        <p className="text-sm text-zinc-600 dark:text-zinc-400">
          The runtime view could not be loaded. Confirm the API is available and
